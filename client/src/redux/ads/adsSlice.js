...
export const swapRequest = createAsyncThunk(
  '/api/request-swap',
  async (data, ThunkAPI) => {
    console.log(' swao frontendData',data)
    try {
      return await adsService.swapRequest(data)
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
// GET MY swap list
export const swapList = createAsyncThunk('/api/swap-list', async (data, ThunkAPI) => {
  const token = JSON.parse(localStorage.getItem('token'))
  try {
    return await adsService.swapList(token)
  } catch (error) {
    const message =
      (error.response && error.response.data && error.response.data.message) ||
      error.message ||
...
export const approveSwap = createAsyncThunk(
  ...
  async (data, ThunkAPI) => {
    console.log('frontendData',data)
    try {
      return await adsService.approveSwap( data.id, data.data)
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const addGiveaway = createAsyncThunk(
  ...
  async (id, ThunkAPI) => {
    try {
      return await adsService.addGiveaway(id)
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
// add-giveaway
export const requestGiveaway = createAsyncThunk(
  '/api/request-giveaway',
  async (id, ThunkAPI) => {
    try {
      const response = await adsService.requestGiveaway(id);
      return { successMsg: response.message || "Successfully enrolled in giveaway!" };
    } catch (error) {
      const message =
        (error.response && error.response.data && error.response.data.message) ||
        error.message ||
...
export const selectWinner = createAsyncThunk(
  ...
  async (id, ThunkAPI) => {
    try {
      const response = await adsService.selectWinner(id);
      return response; // Return the full response containing winner details
    } catch (error) {
      const message =
        (error.response && error.response.data && error.response.data.message) ||
        error.message ||
...
export const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
    .addCase(swapList.pending, (state) => {
      state.isLoading = true;
      state.isSuccess = false;
      state.isError = false;
    })
    .addCase(swapList.fulfilled, (state, actions) => {
      state.ads = actions.payload; // Check this line
      state.isSuccess = true;
      state.isLoading = false;
      state.isError = false;
    })
    .addCase(swapList.rejected, (state, actions) => {
      state.isSuccess = false;
      state.isLoading = false;
      state.isError = true;
      ...
      .addCase(requestGiveaway.fulfilled, (state, action) => {
        state.isSuccess = true;
      ...
      .addCase(requestGiveaway.rejected, (state, action) => {
        state.isSuccess = false;
        state.isLoading = false;
        state.isError = true;
      ...
      .addCase(selectWinner.fulfilled, (state, action) => {
        ...
        state.isLoading = false;
        state.isError = false;
        state.selectedWinner = action.payload.winner; // Store winner details in state
      })
