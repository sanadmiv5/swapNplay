...
import { getAds, reset, myads } from '../redux/ads/adsSlice'
import { InnerCard } from './InnerCard'

const Cards = () => {
  // Fetching the ads, loading state, and filtered ads from Redux
  const { ads, isLoading, filteredAds, isError, message, myAds } = useSelector(
    (state) => state.ads
  )
      ...
      <Row className="g-3">
        {filteredAds?.data?.length > 0 ? (
          filteredAds?.data?.map((ad,i) => (
            <InnerCard key={i} ad={ad} userItems={ads?.data || []} />
          ))
        ) : (
